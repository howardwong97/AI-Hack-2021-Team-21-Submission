...
# Params: [c, phi, theta, omega, alpha, beta]  and v for t-distribution
...
class VolatilityModel:
    ...
    def fit(self, t_dist=False):
        ...
        initial_params = [0.001 for _ in range(3)] + [0.001, 0.1, 0.8]

        if self.t_dist is True:
            bounds.append((3.0, 10.0))
            initial_params.append(3.0)
            result = optimize.fmin_slsqp(
                func=negative_loglikelihood,
                x0=initial_params,
                f_ieqcons=t_ineqcons_func,
                bounds=bounds,
                epsilon=1e-6,
                acc=1e-7,
                full_output=True,
    ...
    def predict(self, log_returns):
        ...
        return (omega + alpha * eps[-1] ** 2 + beta * sigma2[-1]) / 100.0 ** 2
    ...
    def print_summary_stats(self):
        ...
        for i in range(len(self.params)):
            ...
            if self.t_dist :
                llh_neg = t_negative_loglikelihood(self.params - delta, scaled_log_returns)
                llh_pos = t_negative_loglikelihood(self.params + delta, scaled_log_returns)
                scores[:, i] = (llh_pos - llh_neg) / (2 * h)
                V = (scores.T @ scores) / T
                J = hessian_2sided(t_negative_loglikelihood, self.params, scaled_log_returns) / T

            else:
                llh_neg = negative_loglikelihood(self.params - delta, scaled_log_returns)
                llh_pos = negative_loglikelihood(self.params + delta, scaled_log_returns)
                scores[:, i] = (llh_pos - llh_neg) / (2 * h)
                V = (scores.T @ scores) / T
                J = hessian_2sided(negative_loglikelihood, self.params, scaled_log_returns) / T
...
def get_sigma2(omega, alpha, beta, eps):
    T = len(eps)
    sigma2 = np.zeros(T)
...
def negative_loglikelihood(params, r):
    c = params[0]
    phi = params[1]
    theta = params[2]
    omega = params[3]
...
def t_ineqcons_func(params, r):
    phi = params[1]
    theta = params[2]
    alpha = params[4]
    beta = params[5]
    v = params[6]
    return [1 - phi, 1 - theta, 1.0 - alpha - beta, v - 2]
